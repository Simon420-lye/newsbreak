...
def collect():
    ...
    for source, url in FEEDS:
        try:
            ...
            dead.append(source)
            print(f"  [skip] {source}: {e}", flush=True)
            continue
        n = 0
...
def run(output="news_data.json", limit=90):
    """Callable entry point. api.py imports this; the CLI wraps it below."""
    ...
    print(f"\nSaved {len(stories)} stories -> {output}", flush=True)
    if stories:
        print(f"Top story: [{stories[0]['outlets']} outlets] {stories[0]['title'][:70]}", flush=True)
    return payload
